...
import OpenElectiveSubject from "../../course/openelective/model.js";
    ...
    for (const courseData of request.body) {
      ...
      if (missingFields.length > 0) {
        return response
          .status(400)
          .send({
            message: `Missing fields in one or more courses: ${missingFields.join(
              ", "
            )}`,
          });
      }
      ...
      const createdCourse = await OpenElectiveSubject.create(newCourse);
